function calculateGO(){
	...
	if($('#check1')[0].checked){
		stageChecked[0] = true;
		leftFuWen[0] = checkLeft($("[name=fuwen1_LEVEL]")[0].value, $("[name=fuwen1_GROWTH]")[0].value);
	}
	if($('#check2')[0].checked){
		stageChecked[1] = true;
		leftFuWen[1] = checkLeft($("[name=fuwen2_LEVEL]")[0].value, $("[name=fuwen2_GROWTH]")[0].value);
	}
	
}
function checkLeft(level, growth){
